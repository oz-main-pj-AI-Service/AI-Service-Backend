...
class AIRequestTests(APITestCase):
    def setUp(self):
        ...
        self.recipe_url = reverse("ai:recipe-recommendation")  # URL 이름에 따라 다를 수 있음
        ...
        self.recipe_invalid_data = {
            "ingredients": "invalid_string"
        }
    ...
    def test_unauthorized_recipe_request(self):
        response = self.client.post(self.recipe_url, data=self.recipe_valid_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_invalid_data_format(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        response = self.client.post(self.recipe_url, data=self.recipe_invalid_data, format="json")
    ...
    def test_recipe_streaming_mode(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")
        response = self.client.post(self.recipe_url + "?streaming=true", data=self.recipe_valid_data, format="json")
    ...
    def test_unauthorized_food_request(self):
        response = self.client.post(self.food_url, data=self.food_valid_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_valid_food_request(self):
        ...
        response = self.client.post(self.food_url, data=self.food_valid_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn("nutritional_info", response.data["recommendation"]["recommendation"])

    def test_food_streaming_line_by_line(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.access_token}")

        response = self.client.post(
            self.food_url + "?streaming=true",
            data=self.food_valid_data,
            format="json"
        )
    ...
    def test_unauthorized_health_request(self):
        response = self.client.post(self.health_url, data=self.health_valid_data, format="json")
    ...
    def test_valid_health_request(self):
        ...
        self.assertIn("nutritional_info", response.data["meal_plan"]["meals"])
