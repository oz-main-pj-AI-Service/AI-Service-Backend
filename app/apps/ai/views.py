...
def get_test_user():
    try:
        ...
        # 어떤 방법으로도 사용자를 찾지 못했다면 첫 번째 사용자를 반환
    ...
    def post(self, request):
        try:
            ...
            ai_request = AIFoodRequest.objects.create(
                user=get_test_user(), request_type="recipe", request_data=data
            )
...
class HealthBasedRecommendationView(View):
    ...
    def post(self, request):
        try:
            ...
            # AI 요청 데이터 저장
            ai_request = AIFoodRequest.objects.create(
                user=get_test_user(), request_type="health", request_data=data
            )
...
class FoodRecommendationView(View):
    ...
    def post(self, request):
        try:
            ...
            # AI 요청 데이터 저장
            ai_request = AIFoodRequest.objects.create(
                user=get_test_user(), request_type="food", request_data=data
            )
