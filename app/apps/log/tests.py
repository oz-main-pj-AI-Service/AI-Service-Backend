...
class ActivityLogAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.user_log_detail_url = reverse('log:log-list-create', args=[self.user_log.id])
        self.admin_log_detail_url = reverse('log:admin-log-list-create', args=[self.admin_log.id])

    def test_log_list_unauthenticated(self):
        """인증되지 않은 사용자는 로그 목록에 접근할 수 없음"""
        response = self.client.get(self.log_list_url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    
    def test_log_list_regular_user(self):
        ...
        data = response.json()
        self.assertEqual(data['count'], 1)  # 자신의 로그만 조회 가능
        
        # 결과에 다른 사용자 로그가 없는지 확인
        log_ids = [log['id'] for log in data['results']]
        self.assertIn(str(self.user_log.id), log_ids)
        self.assertNotIn(str(self.admin_log.id), log_ids)
    ...
    def test_create_log(self):
        ...
        # 생성된 로그 확인
        created_log = ActivityLog.objects.filter(action=ActivityLog.ActionType.VIEW_REPORT).first()
    ...
    def test_filter_by_action(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get(f"{self.log_list_url}?action={ActivityLog.ActionType.LOGIN}")
    ...
    def test_log_detail_view(self):
        """특정 로그 상세 조회 테스트"""
        self.client.force_authenticate(user=self.user)
        response = self.client.get(self.user_log_detail_url)
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertEqual(data['id'], str(self.user_log.id))
        self.assertEqual(data['action'], self.user_log.action)
        self.assertEqual(data['ip_address'], self.user_log.ip_address)
    
    def test_log_detail_permission(self):
        """다른 사용자의 로그 상세 조회 권한 테스트"""
        self.client.force_authenticate(user=self.user)
        
        # 관리자의 로그 조회 시도
        response = self.client.get(self.admin_log_detail_url)
...
class ActivityLogModelTests(TestCase):
    ...
    def test_str_method(self):
        """__str__ 메서드 테스트"""
        log_str = str(self.logs[0])
        self.assertIn(self.logs[0].action, log_str)
        self.assertIn(f"User {self.user.id}", log_str)
    
    def test_get_user_log_count(self):
        """get_user_log_count 클래스 메서드 테스트"""
        count = ActivityLog.get_user_log_count(self.user.id)
        self.assertEqual(count, 3)  # 설정에서 생성한 로그 3개